...
class PaytmController extends Controller
{
    ...
    { 
       $paytm_requests = PaytmRequest::select('paytm_requests.*')->join('users', 'paytm_requests.user_id', '=', 'users.id')->where('users.status','1')->where('paytm_requests.status','1')->where('paytm_requests.hold_status','1')->orderBy('paytm_requests.id', 'desc')->get();
       ...
       $pending_rupees = PaytmRequest::selectRaw('SUM(paytm_requests.amount) as pending_rupee')->join('users', 'paytm_requests.user_id', '=', 'users.id')->where('users.status','1')->where('paytm_requests.status','1')->orderBy('paytm_requests.id', 'desc')->first(); //echo '<pre>'; print_r($paytm_requests); echo '</pre>';
    ...
    { 
       $paytm_requests = PaytmRequest::select('paytm_requests.*')->join('users', 'paytm_requests.user_id', '=', 'users.id')->where('users.status','1')->where('paytm_requests.status','1')->where('paytm_requests.hold_status','2')->orderBy('paytm_requests.id', 'desc')->get();
       ...
       $pending_rupees = PaytmRequest::selectRaw('SUM(paytm_requests.amount) as pending_rupee')->join('users', 'paytm_requests.user_id', '=', 'users.id')->where('users.status','1')->where('paytm_requests.status','1')->where('paytm_requests.hold_status','2')->orderBy('paytm_requests.id', 'desc')->first(); 
    ...
    {
      $paytm_requests = PaytmRequest::where('status','!=','1') ->whereRaw('DATE(updated_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)')->orderBy('updated_at', 'desc')->get(); 
    ...
    { 
      ...
      if($request->status ==3 || $request->status ==2){
         ...
         Storage::disk('s3')->delete($request->qr_code);
        ...
        if($request->status ==3){
          ...
          Mail::to($user_info)->send(new RejectEmail($user_info,$paytm_request)); 
         ...
         // Mail::to($user_info)->send(new WonEmail($user_info,$paytm_request));
        }
  ...
  // }
    ...
    public function updateoldVersion(){
      ...
      //$paytm_requests_data = PaytmRequest::where('status','1')->orderBy('id', 'desc')->get();
      $paytm_requests_data = PaytmRequest::selectRaw('paytm_requests.*')->join('users','users.id','=','paytm_requests.user_id')->where('users.device_type','1')->where('users.user_app_version','1.0.3')->where('paytm_requests.status','1')->orderBy('id', 'desc')->limit(20)->get();
      $country_code = '91';
      
      
      foreach ($paytm_requests_data as $key => $value) {

        $paytm_request = PaytmRequest::find($value->id);
        $paytm_request->status= '3';
        $paytm_request->save();

        $mobile = $value->paytm_mobile_number;

        $user_info = User::find($value->user_id);
        
        $message = urlencode("Rejected ! PayTm requested Amount of Rs.".$value->amount."/- has been rejected and same has been added to your Wallet. ".$value->remark." For any assistance contact walkearn2019@gmail.com. Order ID. WER".str_pad($value->order_id, 10, 0, STR_PAD_LEFT ).".");
        
        $message1 = "Rejected ! PayTm requested Amount of Rs.".$value->amount."/- has been rejected and same has been added to your Wallet . For any assistance contact walkearn2019@gmail.com. Order ID. WER".str_pad($value->order_id, 10, 0, STR_PAD_LEFT )."."; 
        Mail::to($user_info)->send(new MessageEmail($user_info,$paytm_request));

        $notification_data = ['description' => $message1, 'title' => 'Walk & Earn','type' => 'Wallet'];
        //Helper::sendSms($country_code, $mobile, $message);

        //push notification
        $action = "com.hexalab.walkandearn.activity.WalletActivity";
        if($user_info->device_type=='1'){

          $android_token = User::where('id',$value->user_id)->where('device_type', '1')->get()->pluck('fcm_token')->toArray();
          FcmNotificationController::sendNotificationForPaytm($android_token, $notification_data, $action, '1');
       
        }elseif ($user_info->device_type=='2') {
          
          $ios_token = User::where('id',$value->user_id)->where('device_type', '2')->get()->pluck('fcm_token')->toArray();
          FcmNotificationController::sendNotificationForPaytm($ios_token, $notification_data, $action, '2');
        }
      }
    }
