...
class StepController extends Controller
{
    ...
    public function add_steps_old(Request $request){
      // if (Auth::check()) {
      ...
      if($this->user->id){
        ...
        if(empty($check_user_wallet)){
         ...
         // $slabs_rs = 17;
          ...
          if($reward_id==5){
            $slabs_rs = $this->generate_slab(5,10);
          }

          //slabs if reward id 6 means 700 coins random rupess 5-19
          if($reward_id==6){
            $slabs_rs = $this->generate_slab(4,8);
            //$slabs_rs = 9;
          }
        } 
    ...
    public function toptenUser(){
      ...
      if($this->user->id){
        ...
        $wallet_info = PaytmRequest::selectRaw('paytm_requests.amount,users.id,users.name,users.image')->join('users','users.id','=','paytm_requests.user_id')->where('users.status','1')->where('paytm_requests.status','1')->where('paytm_requests.hold_status','!=','2')->where('users.id','!=',$this->user->id)->inRandomOrder()->limit('20')->get();
        //orderBy('paytm_requests.id','desc')->limit('20')
        
        if(!empty($wallet_info)){
          return response()->json(array('user_data' => $wallet_info,'status' => 'success'), 200);
        }else{
