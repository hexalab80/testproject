...
class StepController extends Controller
{
    ...
    public function add_steps(Request $request){
      ...
      if($user){
          ...
          if($user->user_app_version == '1.0.3'){
            return response()->json(array('message' => 'Please upgrade your App to the new version to sync your steps.'), 422);
          }

          $check_steps_count = Step::where('user_id',$this->user->id)->orderBy('id','desc')->count();
          $check_steps_new = Step::selectRaw('totalsteps')->where('user_id',$this->user->id)->whereDate('sync_date','>=','2019-10-09')->orderBy('id','desc')->first();
          $setting = Setting::find(1);
          $max_step_limit = $setting->max_step_limit;

          if(empty($check_steps_count)){
            $count_step = 0;
          }elseif(!empty($check_steps_new->totalsteps)){
              $count_step = $check_steps_new->totalsteps;
          }else{
            //SELECT sum(steps) FROM `steps` WHERE `user_id` = 3483 and DATE(sync_date) <= '2019-10-05'
            $totalsteps = Step::selectRaw('SUM(steps) as totalsteps')->where('user_id',$this->user->id)->whereDate('sync_date','<=','2019-10-09')->orderBy('id','desc')->first();
            $count_step = $totalsteps->totalsteps;
          }

          $stepsHistories = $request->stepsHistories;
          //return response()->json(array('message' => $stepsHistories), 422); exit;
          foreach ($stepsHistories as $key => $value) {
            ...
            $update_check_list_info = Step::where('timestamp','!=',$timestamp)->where('hour',$hour)->where('sync_date',date('Y-m-d',strtotime($request->date)))->where('user_id',$this->user->id)->orderBy('id','desc')->first();

            $add_check_list_info1 = Step::where('timestamp',$timestamp)->where('hour',$hour)->where('sync_date',date('Y-m-d',strtotime($request->date)))->where('user_id',$this->user->id)->orderBy('id','desc')->first();

            //step 15000 max limit per day
            $checkDaytotalStep = Step::selectRaw('SUM(steps) as daysteps')->where('sync_date',date('Y-m-d',strtotime($request->date)))->where('user_id',$this->user->id)->orderBy('id','desc')->first();

            $last_totalstep = Step::where('user_id',$this->user->id)->where('sync_date',date('Y-m-d',strtotime($request->date)))->orderBy('id','desc')->first();

            if(empty($last_totalstep)){
              $temp_totalstep =0;
            }else{
              $temp_totalstep = $last_totalstep->totalsteps;
            }

            $valid_id = '';

            if(!empty($update_check_list_info)){

              $today_last_step = $update_check_list_info->steps;
              $today_update_totalsteps = $update_check_list_info->totalsteps;
              $previous_last_step = Step::where('user_id',$this->user->id)->where('sync_date',date('Y-m-d',strtotime($request->date. ' -1 day')))->orderBy('id','desc')->first();
              
              if(!empty($previous_last_step->totalsteps)){
                $previous_last_step_info = $previous_last_step->totalsteps;
              }else{
                $previous_last_step_data = Step::selectRaw('SUM(steps) as totalsteps')->where('sync_date','!=',date('Y-m-d'))->where('user_id',$this->user->id)->orderBy('id','desc')->first();
                $previous_last_step_info = $previous_last_step_data->totalsteps;
              }
              $last_actual_entry = $checkDaytotalStep->daysteps - $today_last_step;
              $temp_step = (($last_actual_entry+$steps) > $max_step_limit) ? $max_step_limit : ($last_actual_entry+$steps);

              $update_check_list_info->steps = $steps;
              $update_check_list_info->timestamp = $timestamp;
              $count_step = $update_check_list_info->totalsteps = ($previous_last_step_info + $temp_step);
              $update_check_list_info->updated_at = date('Y-m-d H:i:s');
              $update_check_list_info->save();

              $valid_id = $update_check_list_info->id;

            }elseif(empty($add_check_list_info1)){

              if(empty($checkDaytotalStep)){
                $temp_step = ($steps > $max_step_limit) ? $max_step_limit : $steps;
              }elseif($checkDaytotalStep->daysteps > $max_step_limit){
                  $temp_step =0;
              }else{  
                $temp_step = (($checkDaytotalStep->daysteps + $steps) > $max_step_limit) ? ($max_step_limit - $checkDaytotalStep->daysteps): $steps;
              }
              ...
              $count_step = $list_data->totalsteps = $count_step + $temp_step;
          ...
          if(isset($this->user->frd_referral_code) && !empty($check_valid_user) && ($check_imei_user==1) && ($check_steps_count == 0) && ($valid_id !='')){
            ...
            $previous_last_totalstep = Step::where('user_id',$check_valid_user->id)->orderBy('id','desc')->first();
            $previous_totalcoins =floor($previous_last_totalstep->totalsteps/100);
            $previousgetRewardCoin = RewardCoin::selectRaw('SUM(coins) as total_coin')->where('user_id',$check_valid_user->id)->first();
            $referalUser = User::find($check_valid_user->id);
            $referal_wallet = Wallet::join('rewards','rewards.id','=','wallets.reward_id')->selectRaw("SUM(rewards.value) as redeem_coins")->where('user_id',$check_valid_user->id)->first();
            $referalUser->redeem_coins = $referal_wallet->redeem_coins;
            $referalUser->available_coins = ($previousgetRewardCoin->total_coin+$previous_totalcoins) - $referalUser->redeem_coins;
            $referalUser->save();
          } 
          ...
          $wallet = Wallet::join('rewards','rewards.id','=','wallets.reward_id')->selectRaw("SUM(rewards.value) as redeem_coins")->where('user_id',$this->user->id)->first();
          $user->redeem_coins = $wallet->redeem_coins;
          $get_step = Step::where('sync_date',date('Y-m-d',strtotime($request->date)))->where('user_id',$this->user->id)->orderBy('id','desc')->first();
          $total_steps_data = isset($get_step) ? $get_step->totalsteps : 0;
          ...
          return response()->json(array('message' => 'List has been added successfully.','user' => $user,'todayCoin' => $todayCoin,'totalstepCoin' => $totalcoins), 200); 
         // return response()->json(array('message' => 'List has been added successfully.','user' => $user), 200);
    ...
    public function fetchStep(){
      ...
      if(!empty($user)){
          ...
          if(!empty($list_date_info)){ 
             ...
             $list_date_info->timestamp = ($user->device_type=='2' && strlen($list_date_info->timestamp) ==13) ? (int)(substr($list_date_info->timestamp,0, 10)) : $list_date_info->timestamp;
 ...
 /*
    ...
    public function rewardlist(){
      ...
      $check_timestamp = Wallet::select('created_at')->where('user_id',$this->user->id)->whereDate('redeem_date',date('Y-m-d'))->orderBy('id','desc')->first();
      ...
      $check = Wallet::where('user_id',$this->user->id)->where('reward_id','1')->count();
      if(($check > 0)){
        $new_reward = array();
        foreach ($reward as $key => $value) {
      ...
      return response()->json(array('status' => 'success', 'reward_list' => $new_reward, 'wallet_balance' => $wallet_balance,'total_earning_till_date' => $paytm_bal,'grand_total_bal' => $wallet_info->total_wallet_bal,'user' => $user,'week_start' => $monday,'week_end'=>$sunday,'week_redeem_limit' => $setting->weekly_redem_coin), 200);
      }else{
        return response()->json(array('message' => 'User not found.'), 404);
      }   
    }
      ...
      if($this->user->id){
        ...
        if(empty($check_user_wallet)){
         ...
         // $slabs_rs = 17;
          ...
          if($reward_id==3){
          $slabs_rs = $this->generate_slab(3,5);
          }

          //slabs if reward id 4 means 500 coins random rupess 11-25
          if($reward_id==4){
            $slabs_rs = $this->generate_slab(3,7);
          }

          //slabs if reward id 5 means 1000 coins random rupess 11-25
          if($reward_id==5){
            $slabs_rs = $this->generate_slab(6,11);
          }

          //slabs if reward id 6 means 700 coins random rupess 5-19
          if($reward_id==6){
            $slabs_rs = $this->generate_slab(5,9);
            //$slabs_rs = 9;
          }
        } 

        if(($this->user->available_coins > 0) && ($this->user->available_coins > $reward_info->value)){
          ...
          if($check_count >= $reward_info->value){ 
            ...
            $tempPrediction = TempPrediction::where('user_id',$this->user->id)->where('status',0)->first();
            if(!empty($tempPrediction)){
              $slabs_rs = $slabs_rs+ $tempPrediction->rupee; 
              $tempPrediction->status = 1;
              $tempPrediction->save();
            }
      ...
      if($this->user->id){
          ...
          $wallet_info = Wallet::selectRaw('SUM(rupees) as total_wallet_bal')->where('user_id',$this->user->id)->where('scratch_status','2')->first();
           ...
           if(!empty($wallet_info)){
                ...
                $paytm_txn_info = PaytmRequest::where('user_id',$this->user->id)->where('status',$status)->limit($limit)->offset($offset)->orderBy('id','desc')->get();
    ...
    public function paytmRequest(Request $request){
      ...
      if($this->user->id){
        ...
        $paytm_request_info = PaytmRequest::select('order_id')->where('user_id',$user->id)->where('status','1')->orderBy('id','desc')->first();
        $paid_paytm_request = PaytmRequest::select('id')->where('user_id',$user->id)->where('status','2')->orderBy('id','desc')->first();
        ...
        if(!empty($user) && $user->status == '2'){
          ...
          $paytm_request_info1 = PaytmRequest::select('order_id')->where('user_id',$user->id)->where('status','1')->orderBy('id','desc')->first();
          if(!empty($paytm_request_info1)){
            return response()->json(array('message' => 'Your Request of Ref Id: #'.'WER'.str_pad($paytm_request_info->order_id, 10, 0, STR_PAD_LEFT ).' is in progress. You can request one withdrawal at a time. Please try again later.'), 403);
          }

          if($request->payment_type =='2'){
            return response()->json(array('message' => 'We are facing some issues with Google Pay so please do the payment request via PayTm.'), 403);
          }

          $walletArr = $this->wallet_bal(); 
          if(isset($paid_paytm_request) && $paid_paytm_request->id && $request->amount > 10){
            //Sreturn response()->json(array('message' => 'Minimum withdrawal amount should be Rs.20.','status' => 'error'), 403);
            return response()->json(array('message' => 'Maximum withdrawal amount should be Rs.10.','status' => 'error'), 403);
          }
          ...
          //if($request->amount <= $walletArr['check_amt'] && $request->amount > 0){
            if($request->amount <= $walletArr['wallet_balance'] && $request->amount > 0){

            if($request->amount > 15){
            return response()->json(array('message' => 'Maximum withdrawal amount should be Rs.15.','status' => 'error'), 403);
          } 
            ...
            $paytm_request->payment_type = isset($request->payment_type) ? $request->payment_type: '1';
            ...
            if($image){
              ...
              $file_path = 'qr_code/'.$image_name.'.JPG';
              Storage::disk('s3')->put($file_path, $image, 'public');
            }
            
            $paytm_request->qr_code = $file_path;
            ...
            if($paytm_request->id){
             ...
             return response()->json(array('message' => 'Wallet Balance is not sufficient.','status' => 'error'), 403);
           }
        }
    ...
    public function rewardads(){ 
      ...
      if($this->user->id){
        ...
        $setting = Setting::find(1);
        $last_time_rewardAds= RewardCoin::select('created_at')->where('user_id',$this->user->id)->where('reward_type','2')->orderBy('id','desc')->first();
    ...
    public function getUser(){
      ...
      if($this->user->id){
       ...
       // return response()->json(array('status' => 'success','user' => $user,'condition' => '<ol><li> The amount should be a maximum of 60 percent of the wallet balance.</li><li> It may take up to 24-72 working hours for the amount to get credited in your above mentioned PayTm account.</li><ol>'), 200);
        return response()->json(array('status' => 'success','user' => $user,'condition' => '<ol><li> The amount should be a maximum of Rs.10.</li><li> It may take up to 24-72 working hours for the amount to get credited in your above mentioned PayTm or Google Pay account.</li><ol>'), 200);
      }else{
        return response()->json(array('message' => 'User not found.','status' => 'error'), 404);
      }
    }
    ...
    public function getOrderNumber(){
      ...
      $check_order = PaytmRequest::select('order_id')->orderBy('id','desc')->first();
      $count_num =1;
    ...
    public function frd_referral_code_fun($user_id){ 
      ...
      if($user_id){
       ...
       // $check_user = User::where('frd_referral_code',$user->frd_referral_code)->orWhere('referal_code',$user->frd_referral_code)->count();
        ...
        $check_user = RewardCoin::where('user_id',$user_id)->where('reward_type','1')->count();
    ...
    public function lucky_coupons(Request $request){
      ...
      if($this->user->id){
        ...
        $date = date('Y-m-d');
        $check_user = RewardCoin::select('created_at')->where('user_id',$this->user->id)->where('reward_type','3')->whereDate('created_at','=',$date)->orderBy('id','desc')->first();
    ...
    public function toptenUser(){
      ...
      if($this->user->id){
        ...
        $wallet_info = PaytmRequest::selectRaw('paytm_requests.amount,users.id,users.name')->join('users','users.id','=','paytm_requests.user_id')->where('users.status','1')->where('paytm_requests.status','2')->where('paytm_requests.hold_status','!=','2')->where('users.id','!=',$this->user->id)->where('paytm_requests.amount','>','30')->inRandomOrder()->limit('20')->get();

        $newarr = array();
        foreach ($wallet_info as $key => $value) {
          $amt = PaytmRequest::selectRaw('SUM(paytm_requests.amount) as amount')->where('paytm_requests.status','2')->where('user_id',$value->id)->first();
        ...
        //orderBy('paytm_requests.id','desc')->limit('20')
        
        if(!empty($wallet_info)){
          return response()->json(array('user_data' => $newarr,'status' => 'success'), 200);
        }else{
          return response()->json(array('message' => 'Users not found.','status' => 'error'), 404);
        }
      }else{
          return response()->json(array('message' => 'User not found.','status' => 'error'), 404);
      }
    ...
    public function leaderboard(Request $request){
     ...
     // $limit= $request->limit;
      ...
      $user_info = User::selectRaw('SUM(rewards.value) as weekly_redem_coin')->where('status','1')->limit($limit)->offset($offset)->get();
