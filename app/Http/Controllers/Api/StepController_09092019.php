    ...
    */
    ...
    public function rewardlist(){
      ...
      foreach ($reward as $key => $value) {
        ...
        if($value->value ==300){
          $max_limit = 15*$temp_value;
          $value->description = 'Earn upto ₹'.$max_limit.' from Walk & Earn.';
        }
        if($value->value ==500){
          $max_limit = 25*$temp_value;
          $value->description = 'Earn upto ₹'.$max_limit.' from Walk & Earn.';
        }
    ...
    public function rewardHistory(Request $request){
      ...
      if($this->user->id){
        ...
        $reward = Wallet::selectRaw('wallets.*,rewards.title,rewards.value')->join('rewards','rewards.id','=','wallets.reward_id')->where('user_id',$this->user->id)->limit($limit)->offset($offset)->get();
        if($reward){
      ...
      if($this->user->id){
        ...
        if(!empty($user) && $user->status == '2'){
          ...
          if($request->amount <= $walletArr['check_amt']){
              ...
              return response()->json(array('message' => 'Sorry! you have insufficient balance to add Paytm.','status' => 'error'), 401);
           }
        }
    ...
    public function getUser(){
      ...
      if($this->user->id){
        ...
        $user = User::find($this->user->id); 
        return response()->json(array('status' => 'success','user' => $user,'condition' => '<ol><li> The amount should be a maximum of 80 percent of the wallet balance.</li><li> It may take up to 24-48 working hours for the amount to get credited in your above mentioned PayTm account.</li><ol>'), 200);
      }else{
        return response()->json(array('message' => 'User not found.','status' => 'error'), 404);
      }
    }
