...
class StepController extends Controller
{
    ...
    public function add_steps(Request $request){
      ...
      if($user){
          ...
          $check_steps_count = Step::where('user_id',$this->user->id)->orderBy('id','desc')->count();
          $check_steps_new = Step::where('user_id',$this->user->id)->whereDate('sync_date','>=','2019-10-09')->orderBy('id','desc')->first();
          $setting = Setting::find(1);
  ...
  public function toptenUser(){
    ...
    if($this->user->id){
      ...
      $wallet_info = PaytmRequest::selectRaw('paytm_requests.amount,users.id,users.name,users.image')->join('users','users.id','=','paytm_requests.user_id')->where('users.status','1')->where('paytm_requests.status','1')->where('users.id','!=',$this->user->id)->inRandomOrder()->limit('20')->get();
